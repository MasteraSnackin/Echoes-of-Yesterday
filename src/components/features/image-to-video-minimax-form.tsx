...
import { imageToVideoMinimaxAction } from '@/app/actions';
...
export function ImageToVideoMinimaxForm() {
  ...
  const handleGenerate = async () => {
    ...
    const result = await imageToVideoMinimaxAction({
      imageUrl,
      prompt,
      apiKey,
    });
  ...
  return (
    <div className="grid gap-8 md:grid-cols-2">
      <Card>
        <CardHeader>
          <CardTitle className="font-headline text-2xl">Image-to-Video with Minimax</CardTitle>
          <CardDescription>
            Generate a short video from an image and an optional prompt using the Fal.ai Minimax API. Videos can take a few minutes to generate.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              id="prompt"
              placeholder="e.g., A stylish woman walks down a Tokyo street..."
              value={prompt}
